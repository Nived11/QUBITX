...
const AddressList = ({
  ...
  return (
    <div className="min-h-screen max-h-screen overflow-y-auto scrollbar-hide space-y-4 pr-2">
      {addresses.length === 0 ? (
        ...
        addresses.map((address) => {
          const isSelected = selectionMode && selectedAddress?._id === address._id;

          return (
            <div
              key={address._id}
              onClick={() => handleCardClick(address)}
              className={`bg-white rounded-lg p-4 sm:p-6 border-2 transition relative ${selectionMode
                  ? isSelected
            ...
            >
              ...
              {/* Address Type Badge & Selected Badge */}
