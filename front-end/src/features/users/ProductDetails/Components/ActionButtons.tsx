    ...
    <>
      ...
      <div className="hidden sm:grid grid-cols-2 gap-3 flex-shrink-0 px-4 mb-6">
        <button
          ...
          disabled={isAdding || (isOutOfStock && !shouldShowGoToCart)}
          className={`text-sm flex justify-center items-center gap-2 ${
            ...
            isAdding || (isOutOfStock && !shouldShowGoToCart)
        ...
        <button
          ...
          className={`text-sm sm:text-base bg-gradient-to-r from-blue-800 to-blue-900 text-white py-3 rounded-md font-semibold transition shadow-[0_4px_10px_rgba(0,0,0,0.3)] ${
            isOutOfStock
              ? "opacity-50 cursor-not-allowed"
              : "hover:opacity-90"
          }`}
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white shadow-lg px-3 py-2 z-50">
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            onClick={onBuyNow}
            disabled={isOutOfStock}
            className={`text-xs bg-gradient-to-r from-blue-800 to-blue-900 text-white py-3 rounded-md font-semibold transition ${
              isOutOfStock
                ? "opacity-50 cursor-not-allowed"
                : "hover:opacity-90"
            }`}
